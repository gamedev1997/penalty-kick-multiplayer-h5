...
/* Celebration overlay — fixed to viewport so it never clips off-screen */
.mph-celebration-container {
    position: fixed;
    bottom: clamp(90px, 16vmin, 160px); /* above the HUD cards */
    pointer-events: none;
    display: none;
    justify-content: center;
    align-items: flex-end;
    ...
    max-height: calc(100vh - clamp(100px, 18vmin, 180px)); /* never exceed viewport */
}
.mph-cel-left {
    left: clamp(15px, 6vmin, 80px);
}
.mph-cel-right {
    right: clamp(15px, 6vmin, 80px);
}
.mph-celebration-img {
    width: 100%;
    height: 100%;
...
@media (max-width: 768px) {
    ...
    .inner-left-align {
        padding-left: clamp(68px, 13vmin, 88px);
        padding-right: clamp(24px, 5vmin, 36px);
        gap: clamp(1px, 0.3vmin, 3px);
    }
    
    .inner-right-align {
        padding-right: clamp(68px, 13vmin, 88px);
    ...
    function _buildDOM() {
        // Clean up any existing duplicate HUD in the DOM
        var existingHud = document.getElementById('mp-hud');
        ...
        _el.innerHTML =
            ...
            // Celebration containers — fixed position, direct children of HUD
            '  <div class="mph-celebration-container mph-cel-left" id="mph-left-celebration">' +
           szLeftCelebrationHtml +
            '  </div>' +
            '  <div class="mph-celebration-container mph-cel-right" id="mph-right-celebration">' +
           szRightCelebrationHtml +
    '  </div>';

        document.body.appendChild(_el);

        document.getElementById('mph-my-name').textContent = _formatName(_szMyName);
        document.getElementById('mph-opp-name').textContent = _formatName(_szOppName);
    ...
    function _renderDots(id, shots, max) {
        ...
        // Safety: ensure max is at least as many as shots already taken
        if (!max || max < 1) max = 10;
        if (shots.length > max) max = shots.length;
    ...
    this.init = function (myName, oppName, maxShots, myAvatar, oppAvatar) {
        ...
        _iMaxShots = parseInt(maxShots, 10) || 10;
        if (_iMaxShots < 5) _iMaxShots = 5;
