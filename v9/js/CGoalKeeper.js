function CGoalKeeper(iXPos, iYPos, oParentContainer) {
    ...
    this.setPosition = function (iXPos, iYPos) {
        console.log('[GK] setPosition x=' + iXPos + ' y=' + iYPos);
        _oContainer.x = iXPos;
        _oContainer.y = iYPos;
    };
    ...
    this.fadeAnimation = function (fVal) {
        console.log('[GK] fadeAnimation → alpha=' + fVal + ' (over 500ms), current alpha=' + _oContainer.alpha);
        createjs.Tween.get(_oContainer, {override: true}).to({alpha: fVal}, 500);
    };

    this.setAlpha = function (fVal) {
        console.log('[GK] setAlpha → ' + fVal);
    ...
    this.runAnim = function (iVal) {
        console.log('[GK] runAnim → anim=' + iVal);
        this.disableAllAnim();
        ...
        // Force cache update immediately so the rendered bitmap shows the correct
        // frame right away — prevents stale dive frame flashing during fade-in.
        _oContainer.updateCache();
    };
    ...
    this.runAnimAndShift = function (iVal, pBallFinalPos, fAnticTime, fDiveTime, fAnimSpeedScale, fErrorX, fErrorY) {
        console.log('[GK] runAnimAndShift anim=' + iVal + ' ballFinalPos=(' + pBallFinalPos.x.toFixed(1) + ',' + pBallFinalPos.y.toFixed(1) + ') antic=' + fAnticTime + ' dive=' + fDiveTime + ' speed=' + fAnimSpeedScale);
        if (_bDirectionLocked) { console.log('[GK] runAnimAndShift BLOCKED (directionLocked)'); return; }
        ...
        console.log('[GK] runAnimAndShift → shifting to x=' + (_pStartPos.x + iX).toFixed(1) + ' y=' + (_pStartPos.y + iY).toFixed(1) + ' (from start ' + _pStartPos.x + ',' + _pStartPos.y + ')');
        ...
        createjs.Tween.get(_oContainer, { override: true })
            ...
            .call(function () {
                console.log('[GK] runAnimAndShift tween DONE → STATE_GK_SAVE_CONTACT, pos=(' + _oContainer.x.toFixed(1) + ',' + _oContainer.y.toFixed(1) + ')');
    ...
    this.runAnimWrongDir = function (iVal, fAnticTime, fDiveTime, fAnimSpeedScale) {
        console.log('[GK] runAnimWrongDir anim=' + iVal + ' antic=' + fAnticTime + ' dive=' + fDiveTime + ' speed=' + fAnimSpeedScale);
        if (_bDirectionLocked) { console.log('[GK] runAnimWrongDir BLOCKED (directionLocked)'); return; }
        _bDirectionLocked = true;
        ...
        console.log('[GK] runAnimWrongDir → shifting to x=' + (_pStartPos.x + iShiftX).toFixed(1) + ' y=' + (_pStartPos.y + iShiftY).toFixed(1));
        ...
        createjs.Tween.get(_oContainer, { override: true })
            ...
            .call(function () {
                console.log('[GK] runAnimWrongDir tween DONE → pos=(' + _oContainer.x.toFixed(1) + ',' + _oContainer.y.toFixed(1) + ')');
                _iState = STATE_GK_SAVE_CONTACT;
            });
    ...
    this.resetToStartPosition = function () {
        console.log('[GK] resetToStartPosition → x=' + _pStartPos.x + ' y=' + _pStartPos.y + ' (was x=' + _oContainer.x.toFixed(1) + ' y=' + _oContainer.y.toFixed(1) + ')');
