...
var CMultiplayer = (function () {
    ...
    function connectThenRequest(playerName, mode, roomCode) {
        ...
        function _doRequest() {
            if (mode === 'random') {
                ...
                _send(MsgType.REQUEST_ROOM, { playerName: myName, mode: Mode.RANDOM, payAmount: payAmount });
            } else if (mode === 'create') {
                currentMode = Mode.PRIVATE;
                _send(MsgType.REQUEST_PRIVATE_ROOM, { playerName: myName, roomCode: roomCode, payAmount: payAmount });
            } else if (mode === 'join') {
                currentMode = Mode.PRIVATE;
                _send(MsgType.REQUEST_PRIVATE_ROOM, { playerName: myName, roomCode: roomCode, payAmount: payAmount });
            }
    ...
    function requestBot() {
        ...
        _send(MsgType.REQUEST_ROOM, { playerName: myName, mode: Mode.VS_BOT, payAmount: payAmount });
    }
    ...
    function _getPayAmount() {
        if (!window.GAME_INIT_DATA || window.GAME_INIT_DATA.payAmount === undefined || window.GAME_INIT_DATA.payAmount === null) {
            console.error('[CMultiplayer] GAME_INIT_DATA missing or payAmount not set:', window.GAME_INIT_DATA);
            return null;
        }
    ...
    function createPrivateRoom(code) {
        ...
        if (payAmount === null) return;
        currentMode = Mode.PRIVATE;
        _send(MsgType.REQUEST_PRIVATE_ROOM, { playerName: myName, roomCode: code, isCreator: true, payAmount: payAmount });
    }
    ...
    function joinPrivateRoom(code) {
        ...
        if (payAmount === null) return;
        currentMode = Mode.PRIVATE;
        _send(MsgType.REQUEST_PRIVATE_ROOM, { playerName: myName, roomCode: code, isCreator: false, payAmount: payAmount });
    }
