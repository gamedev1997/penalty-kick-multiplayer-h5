function CScoreBoard(oParentContainer) {
    ...
    this.refreshMultiplayerScore = function (iMyGoals, iOppGoals, szMyName, szOppName) {
        var myLabel  = szMyName  || 'YOU';
        var oppLabel = szOppName || 'OPP';
        var szText   = myLabel + ': ' + iMyGoals + '  |  ' + oppLabel + ': ' + iOppGoals;
        if (!_oMpLabel) {
            _oMpLabel = new createjs.Text(szText, '32px ' + FONT_GAME, '#ffffff');
            _oMpLabel.textAlign = 'center';
            _oMpLabel.x = CANVAS_WIDTH_HALF;
            _oMpLabel.y = CANVAS_HEIGHT - 50;
            _oMpLabel.shadow = new createjs.Shadow('#000000', 1, 1, 4);
            _oParentContainer.addChild(_oMpLabel);
        } else {
            _oMpLabel.text = szText;
        }
    };
