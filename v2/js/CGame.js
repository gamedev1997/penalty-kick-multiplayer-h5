function CGame(oData, gameStartData) {
    ...
    this.handSwipeAnim = function () {
        if (_bLaunched || _bHelpTextHidden) {
            // Ball launched or kick already performed: hide help text immediately
            if (_oInterface && _oInterface.showHelpText) { _oInterface.showHelpText(false); }
            return;
        }
        if (_bMultiplayerMode && (!_bIsMyTurn || _bReplayingKick)) {
            // Opponent's turn or replaying opponent's kick: hide cursor + help text
        ...
        // Only show the help text if NOT in multiplayer, or it's definitely my turn
    ...
    this._createMpHUD = function (szMyName, szOppName) {
        ...
        _oMpHUD = new CMpHUD();
        _oMpHUD.init(_sMpMyName, _sMpOppName, iMax);
        s_oMpHUD = _oMpHUD;
    };
    ...
    this.onMpRoundResult = function (data) {
        ...
        if (bIWasKicker) {
            ...
            if (data.scored) {
                ...
                // Ball was actually kicked → animate the save normally
                if (!_bSaved && _bLaunched) {
                    setTimeout(function () {
                        if (!_bSaved) {
                            _bSaved = true;
                            self._mpSaveBall();
                    ...
                    // Timeout: ball never moved — no deflection, just show MISSED
                    if (_oMpHUD) { _oMpHUD.showResult('MISSED'); }
                    // HUD will be restored to turn state after 2.5s by onMpRoundResult flow
                }
            }
        } else {
            // -- OPPONENT WAS THE KICKER --
            if (_bOppTimedOut || data.timeout) {
                // Opponent timed out: skip ball animation entirely
                _bOppTimedOut = false;
                var hud = _oMpHUD || s_oMpHUD;
                if (hud) { hud.showResult('OPP TIMEOUT'); }
            } else {
                // Normal kick — replay their animation
