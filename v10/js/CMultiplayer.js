...
var CMultiplayer = (function () {
    ...
    // ── Config ────────────────────────────────────────────────────────────────
    // var SERVER_URL = (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' || window.location.protocol === 'file:')
    //     ? 'http://localhost:9560'
    //     : 'https://multiplayer-penalty-app.game2wins.com/';
    ...
    // ── Public API ────────────────────────────────────────────────────────────
    function connectThenRequest(playerName, mode, roomCode) {
        myName = playerName || 'Player';
        var payAmount = _getPayAmount();
        if (payAmount === null) {
            console.error('[CMultiplayer] Cannot request — GAME_INIT_DATA.payAmount missing');
            return;
        }

        function _doRequest() {
            var avatarFile = (typeof CAvatarManager !== 'undefined') ? CAvatarManager.getMyAvatar() : null;
            if (mode === 'random') {
                currentMode = Mode.RANDOM;
                _send(MsgType.REQUEST_ROOM, { playerName: myName, mode: Mode.RANDOM, payAmount: payAmount, avatar: avatarFile });
            } else if (mode === 'create') {
                currentMode = Mode.PRIVATE;
                _send(MsgType.REQUEST_PRIVATE_ROOM, { playerName: myName, roomCode: roomCode, payAmount: payAmount, avatar: avatarFile });
            } else if (mode === 'join') {
                currentMode = Mode.PRIVATE;
                _send(MsgType.REQUEST_PRIVATE_ROOM, { playerName: myName, roomCode: roomCode, payAmount: payAmount, avatar: avatarFile });
            } else if (mode === 'bot') {
                currentMode = Mode.VS_BOT;
                _send(MsgType.REQUEST_ROOM, { playerName: myName, mode: Mode.VS_BOT, payAmount: payAmount, avatar: avatarFile });
            }
        }
        ...
        if (socket && socket.connected) {
            ...
            socket.once('connect', function () {
                isConnected = true;
                console.log('[CMultiplayer] Connected. payAmount=' + payAmount + ' mode=' + mode);
                _doRequest();
            });
        }
    }
    ...
    function requestBot(playerName) {
        if (playerName) {
            myName = playerName;
        }
        currentMode = Mode.VS_BOT;
        var payAmount = (window.GAME_INIT_DATA && typeof window.GAME_INIT_DATA.payAmount !== 'undefined') ? window.GAME_INIT_DATA.payAmount : 0;
        var avatarFile = (typeof CAvatarManager !== 'undefined') ? CAvatarManager.getMyAvatar() : null;
        _send(MsgType.REQUEST_ROOM, { playerName: myName, mode: Mode.VS_BOT, payAmount: payAmount, avatar: avatarFile });
    }

    function _getPayAmount() {
        if (!window.GAME_INIT_DATA || window.GAME_INIT_DATA.payAmount === undefined || window.GAME_INIT_DATA.payAmount === null) {
            console.warn('[CMultiplayer] GAME_INIT_DATA missing or payAmount not set. Falling back to 1.');
            return 1;
        }
        return Number(window.GAME_INIT_DATA.payAmount);
    }
    ...
    function createPrivateRoom(code) {
        ...
        currentMode = Mode.PRIVATE;
        var avatarFile = (typeof CAvatarManager !== 'undefined') ? CAvatarManager.getMyAvatar() : null;
        _send(MsgType.REQUEST_PRIVATE_ROOM, { playerName: myName, roomCode: code, isCreator: true, payAmount: payAmount, avatar: avatarFile });
    }
    ...
    function joinPrivateRoom(code) {
        ...
        currentMode = Mode.PRIVATE;
        var avatarFile = (typeof CAvatarManager !== 'undefined') ? CAvatarManager.getMyAvatar() : null;
        _send(MsgType.REQUEST_PRIVATE_ROOM, { playerName: myName, roomCode: code, isCreator: false, payAmount: payAmount, avatar: avatarFile });
    }
    ...
    function sendTurnTimeout() {
        ...
        _send(MsgType.GAME_EVENT, { event: Events.PLAYER_KICK, direction: randomDir, isTimeout: true });
    }
    ...
    function startTimer(duration) {
        ...
        timeLeft = (typeof duration === 'number') ? duration : TURN_TIMER_DURATION;
    ...
    function getMyAvatar()      { return myAvatar; }
    function getOpponentAvatar(){ return opponentAvatar; }
    ...
    function _onGameEvent(data) {
        switch (data.event) {
            ...
            case Events.GAME_START:
                ...
                // resolve opponent and player names and avatars from players list
                if (data.players) {
                    for (var i = 0; i < data.players.length; i++) {
                        if (data.players[i].playerId != myPlayerId) {
                            opponentName = data.players[i].name;
                            opponentAvatar = data.players[i].avatar || null;
                            // Store opponent avatar in CAvatarManager
                            if (typeof CAvatarManager !== 'undefined') {
                                CAvatarManager.setOppAvatar(opponentAvatar);
                            }
                        } else {
                            myName = data.players[i].name;
                            myAvatar = data.players[i].avatar || null;
                            // Store my avatar in CAvatarManager
                            if (typeof CAvatarManager !== 'undefined') {
                                CAvatarManager.setMyAvatar(myAvatar);
                            }
                        }
                    }
                }
    ...
    return {
        ...
        connectThenRequest: connectThenRequest,
        ...
        getOpponentAvatar: getOpponentAvatar,
