...
function CModeSelect(onModeReady) {
    ...
    this._init = function () {
        ...
        _el.innerHTML = [
            ...
            '    <div class="ms-avatar-container" id="ms-avatar-container" style="margin-top: 10px; margin-bottom: 5px;"></div>',
            ...
            '      <div class="ms-waiting-avatar-container" id="ms-waiting-avatar-container" style="margin-bottom: 5px;"></div>',
            ...
            '        <button class="ms-copy-btn" onclick="s_oModeSelect._copyRoomCode()">📋 Copy</button>',
            '      </div>',
        ...
        if (typeof CAvatarManager !== 'undefined') {
            if (!window.GAME_INIT_DATA) {
                CAvatarManager.setMyAvatar(null);
            }
            CAvatarManager.resetOppAvatar();
            CAvatarManager.preloadAll();
            this._updateAvatarPreviews();
        }

        // Check if game init data is already here, or wait for late arrival
        var self = this;
        if (window.GAME_INIT_DATA) {
            this._handleAutoInit();
        } else {
            window.addEventListener('message', function (event) {
                const data = event.data;
                if (data && data.type === 'GAME_INIT') {
                    setTimeout(function() {
                        self._handleAutoInit();
                    }, 50);
                }
            });
        }
    ...
    this._showFriend = function () {
        ...
        // Hide Create Room option in SDK mode
        if (window.GAME_INIT_DATA) {
            var createBtn = _el ? _el.querySelector('.ms-btn-create') : null;
            if (createBtn) createBtn.style.display = 'none';
            var divider = _el ? _el.querySelector('.ms-divider') : null;
    ...
    this._showWaiting = function (roomCode) {
        ...
        var avatarContainer = document.getElementById('ms-waiting-avatar-container');
        if (avatarContainer && typeof CAvatarManager !== 'undefined') {
            avatarContainer.innerHTML = CAvatarManager.renderAvatarImg(CAvatarManager.getMyAvatarPath(), 'ms-waiting-avatar');
        }
        ...
        if (hint) hint.textContent = roomCode ? 'Share this code with your friend to join' : 'Looking for a match...';
    };
    ...
    this._onBot = function () {
        ...
        s_oMultiplayer.connectThenRequest(name, 'bot', null);
    };
    ...
    this._onRandom = function () {
        ...
        s_oMultiplayer.connectThenRequest(name, 'random', null);
    };
    ...
    this._onCreateRoom = function () {
        ...
        s_oMultiplayer.onGameStart = function (data) { s_oModeSelect._startGame(data); };
        this._setupCommonCallbacks();
        s_oMultiplayer.connectThenRequest(name, 'create', code);
    };
    ...
    this._onJoinRoom = function () {
        ...
        s_oMultiplayer.onGameStart = function (data) { s_oModeSelect._startGame(data); };
        this._setupCommonCallbacks();
        s_oMultiplayer.connectThenRequest(name, 'join', code);
    };
    ...
    this._onJoinRoomWithCode = function (code) {
        var name = this._getName();
        this._showWaiting(null);
        s_oMultiplayer.onGameStart = function (data) { s_oModeSelect._startGame(data); };
        this._setupCommonCallbacks();
        s_oMultiplayer.connectThenRequest(name, 'join', code.toUpperCase());
    };

    this._handleAutoInit = function () {
        if (!_el) return; // Already started
        const initData = window.GAME_INIT_DATA;
        if (!initData) return;

        console.log("⚙️ SDK Game Init Data detected. Processing auto-start...");

        var playerName = 'Player';
        if (initData.user) {
            playerName = initData.user.name || initData.user.username || 'Player';
        }
        
        var nameInput = document.getElementById('ms-name');
        if (nameInput) {
        ...
        var gameType = (initData.gameType || initData.mode || '').toLowerCase();
        var roomCode = (initData.roomNumber || initData.roomCode || null);
        if (roomCode) roomCode = String(roomCode).toUpperCase().trim();

        var isFriendMode = (gameType === 'private' || gameType === 'friend');
        var isBotMode    = (gameType === 'bots' || gameType === 'bot' || gameType === 'vs_bot');

        if (isBotMode) {
            console.log('Auto-selecting mode: VS BOT');
            this._onBot();

        } else if (isFriendMode) {
            if (!roomCode) {
                console.warn('Play with Friend: roomNumber missing in GAME_INIT');
                return;
            }
            console.log('Auto-selecting mode: PLAY WITH FRIEND, code=' + roomCode);
            this._showWaiting(roomCode);
            s_oMultiplayer.onWaiting = function () {};
            s_oMultiplayer.onGameStart = function (data) { s_oModeSelect._startGame(data); };
            this._setupCommonCallbacks();
            s_oMultiplayer.connectThenRequest(playerName, 'join', roomCode);

        } else {
            // gameType === 'online' or anything else → random matchmaking
            console.log('Auto-selecting mode: RANDOM MATCHMAKING');
            this._onRandom();
        }
    };

    this._copyRoomCode = function () {
        var code = document.getElementById('ms-room-code-val').textContent;
        if (!code) return;
        navigator.clipboard.writeText(code).then(function () {
            var btn = document.querySelector('.ms-copy-btn');
    ...
    this._updateAvatarPreviews = function () {
        var container = document.getElementById('ms-avatar-container');
        if (container && typeof CAvatarManager !== 'undefined') {
            container.innerHTML = CAvatarManager.renderAvatarImg(CAvatarManager.getMyAvatarPath(), 'ms-avatar-preview');
        }
    };
...
.ms-copy-btn {
    ...
    border: 1px solid rgba(255,215,0,0.3); transition: background 0.2s;
}
.ms-copy-btn:hover { background: rgba(255,215,0,0.25); }
