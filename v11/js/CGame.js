function CGame(oData, gameStartData) {
    ...
    this._init = function () {
        ...
        // Sensor: x=±20.5, y=132 (center), z=-9 (field) to z=4.3 (crossbar bottom)
        var oGoalAreaDebug = new createjs.Shape();
        var ptBL = this.convert3dPosTo2dScreen({ x: -BACK_WALL_GOAL_SIZE.width, y: 132, z: -9 }, _oCamera);
        var ptBR = this.convert3dPosTo2dScreen({ x:  BACK_WALL_GOAL_SIZE.width, y: 132, z: -9 }, _oCamera);
        var fCrossbarBottomZ = BACK_WALL_GOAL_POSITION.z + POLE_RIGHT_LEFT_SIZE.height * 0.5 - POLE_RIGHT_LEFT_SIZE.radius_top; // ≈ 4.3
        var ptTR = this.convert3dPosTo2dScreen({ x:  BACK_WALL_GOAL_SIZE.width, y: 132, z: fCrossbarBottomZ }, _oCamera);
        var ptTL = this.convert3dPosTo2dScreen({ x: -BACK_WALL_GOAL_SIZE.width, y: 132, z: fCrossbarBottomZ }, _oCamera);
    ...
    this.areaGoal = function () {
        ...
        if (!_bGoal && !_bSaved) {
            // The ball reached the goal sensor — it is a GOAL!
            // Also clear any active pole-collision timer: if the ball clipped a post and
            // deflected INTO the goal, the goal must take priority over the 2500ms pole-reset
            // countdown that would otherwise show MISSED.
            _bGoal = true;
            _bPoleCollide = false;      // cancel pole-reset countdown so timeReset() wins
            _fTimePoleReset = 0;        // ensure no residual countdown
    ...
    this.ballOut = function () {
        ...
        if (!_bBallOut && !_bGoal && !_bSaved) {
            ...
            // ── Anti-tunneling failsafe ─────────────────────────────────────────────
            // If the ball has crossed deep into the goal area (y >= 133.5) but the
            // physics sensor was never triggered (tunneling at high speed), check
            // whether the ball is genuinely inside the goal opening.  If it is,
            // register it as a GOAL rather than a MISS.
            // Goal opening bounds (physics):
            //   X : ±BACK_WALL_GOAL_SIZE.width  (±20.5)
            //   Z : field level (-9) to just below crossbar bottom
            //         = BACK_WALL_GOAL_POSITION.z + POLE_RIGHT_LEFT_SIZE.height*0.5
            //           - POLE_RIGHT_LEFT_SIZE.radius_top
            //         = -2.7 + 7.5 - 0.5  = 4.3
            if (oPos.y >= 133.5) {
                var fCrossbarBottomZ = BACK_WALL_GOAL_POSITION.z
                                     + POLE_RIGHT_LEFT_SIZE.height * 0.5
                                     - POLE_RIGHT_LEFT_SIZE.radius_top;  // ≈ 4.3
                var bInsideGoalX = (Math.abs(oPos.x) <= BACK_WALL_GOAL_SIZE.width + BALL_RADIUS);
                var bInsideGoalZ = (oPos.z <= fCrossbarBottomZ + BALL_RADIUS);

                if (bInsideGoalX && bInsideGoalZ) {
                    // Ball is inside the goal frame — sensor was tunneled through.
                    // Treat as GOAL (visual goal must always be counted).
                    console.warn('[ballOut] Anti-tunneling failsafe fired: registering GOAL at y='
                                 + oPos.y.toFixed(2) + ' x=' + oPos.x.toFixed(2) + ' z=' + oPos.z.toFixed(2));
                    _bGoal = true;
                    _bPoleCollide = false;      // cancel pole-reset countdown so timeReset() wins
                    _fTimePoleReset = 0;        // ensure no residual countdown
                    _fTimeReset = TIME_RESET_AFTER_GOAL;
                    this.textGoal();
                    if (_iArea >= 0) { this.calculateScore(); }
                    playSound("goal", 1, false);
                    return;
                }
                // Ball is past the goal line but outside the goal frame — genuine miss.
                _bBallOut = true;
                _fTimeReset = TIME_RESET_AFTER_BALL_OUT;
                _oInterface.createAnimText(TEXT_BALL_OUT, 90, false, TEXT_COLOR_1, TEXT_COLOR_STROKE);
                playSound("ball_saved", 1, false);
            ...
            // ── Miss check: ball went wide past the goal plane without hitting the post ──
            // Only trigger on X-overflow when ball has reached the goal line (y >= 130)
            // and we are not in a pole collision state (which allows deflection).
            if (!_bPoleCollide && oPos.y >= 130 && (oPos.x > BACK_WALL_GOAL_SIZE.width || oPos.x < -BACK_WALL_GOAL_SIZE.width)) {
                _bBallOut = true;
                _fTimeReset = TIME_RESET_AFTER_BALL_OUT;
                _oInterface.createAnimText(TEXT_BALL_OUT, 90, false, TEXT_COLOR_1, TEXT_COLOR_STROKE);
                playSound("ball_saved", 1, false);
