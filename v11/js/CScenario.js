function CScenario() {
    ...
    this._init = function () {
        ...
        if (SHOW_AREAS_GOAL) {
            ...
            // FIX: Use a deeper sensor (depth=5, 10 units total) to prevent high-speed ball
            // tunneling. The original depth=1 (2-unit box) was smaller than the maximum ball
            // Y-travel per physics substep (~2.2 units at full power), causing the sensor to
            // be skipped entirely. The sensor is centred at y=132 and now spans y=122→142,
            // safely between GK_INTERCEPT_Y(126) and BACK_WALL_GOAL_POSITION.y(155).
            var oGoalSensorSize = {
                width  : BACK_WALL_GOAL_SIZE.width,   // x: ±20.5  (unchanged)
                depth  : 8,                            // y: ±8  → spans y=124→140  (was ±1 → y=131→133)
                height : BACK_WALL_GOAL_SIZE.height    // z: ±7.5 (unchanged)
            };
            s_oScenario.createAreaGoal(GOAL_LINE_POS, oGoalSensorSize, COLOR_AREA_GOAL[0], null);
        }
...
//        });
    ...
    this.createAreaGoal = function (oPos, oProperties, oColor, oInfo) {
        ...
        oGoalBodyLine.collisionResponse = false;
    ...
    this.lineGoalCollision = function (e) {
        var oUserData = null;
        if (e.target && e.target.userData !== undefined) {
            oUserData = e.target.userData;
        } else if (e.contact && e.contact.bj && e.contact.bj.userData !== undefined) {
            oUserData = e.contact.bj.userData;
        }
        s_oGame.areaGoal(oUserData);
    };
