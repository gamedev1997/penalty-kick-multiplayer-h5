function CGame(oData, gameStartData) {
    ...
    this._init = function () {
        ...
        // ── Custom banner overlay to mask advertisement banners ───────────────
        // This sits on top of the background but behind the goal, GK, ball, etc.
        (function () {
            var iBannerY = 200;   // top edge of the ad banner strip
            var oBanner = createBitmap(s_oSpriteLibrary.getSprite("bannerLogo"));
            oBanner.x = 0;
            oBanner.y = iBannerY;
            _oContainerGame.addChild(oBanner);
        })();
    ...
    this._updateTurnState = function () {
        ...
        if (_bIsMyTurn) {
            ...
            var self = this;
            var iOppTime = 15;
            var iGraceTime = 1; // 4 seconds grace period to allow server latency
    ...
    this.onMpRoundResult = function (data) {
        ...
        if (_bIsMyTurn !== bIWasKicker) {
            console.warn('[MP] Received stale/incorrect round result. Expected kicker is ' + (_bIsMyTurn ? 'me' : 'opponent') + ' but result kicker is ' + (bIWasKicker ? 'me' : 'opponent') + '. Ignoring.');
            return;
        }
